...
export default function TagInput({ 
    ...
    useEffect(() => {
        if (query.trim().length === 0) {
            setSuggestions(allTags.filter(t => !selectedTags.some(st => st.id === t.id)));
            return;
        }
    ...
    const canCreateTag = query.trim().length >= 2 && 
    ...
    return (
        <div className="relative">
            ...
            {showDropdown && selectedTags.length < maxTags && (
                ...
                >
                    {isLoading ? (
                        <div className="p-3 text-center text-gray-400 text-sm">Chargement...</div>
                    ) : (
                        <>
                            {suggestions.map(tag => (
                                <button
